...
# CSS: Top alignment, hiding Streamlit overlays, and custom MD3 theme
st.markdown("""
<style>
    /* Completely hide top header, toolbar, footer, and developer overlays */
    ...
    /* Force app content to top of mobile screen */
...
KNOWN_SDKS = ["firebase", "unity", "flutter", "unreal", "admob", "facebook", "graphql", "sentry", "bugsnag", "appsflyer"]
...
def inspect_entire_bundle(file_bytes):
    """Scans all files, C++ native libraries, services, SDKs, and endpoints."""
    details = {
        "files": set(),
        "total_size": 0,
        "all_strings": set(),
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(file_bytes), "r") as z:
            for name in z.namelist():
                ...
                # Check for third-party SDKs in paths
                for sdk in KNOWN_SDKS:
                    if sdk in lower_name:
                        details["detected_sdks"].add(sdk.capitalize())
...
if st.session_state.report_html:
    ...
    with col_a:
        st.markdown('<div class="title-text">📊 Teardown Report</div>', unsafe_allow_html=True)
    with col_b:
    ...
    # Optional Visual Asset Previews
...
# ==================== MAIN INPUT VIEW ====================
    ...
    st.markdown('<div class="title-text">📱 Universal APK Teardown</div>', unsafe_allow_html=True)
    st.markdown('<div class="sub-text">Deep-scans C++ binaries, game configs, services, and endpoints into an instant report.</div>', unsafe_allow_html=True)
    ...
    if st.button("🚀 Run Deep Package Teardown", type="primary", use_container_width=True):
        if "GROQ_API_KEY" not in st.secrets or not st.secrets["GROQ_API_KEY"]:
            ...
            <div class="scanner-box">
                ...
                <div style="font-weight: bold; font-size: 15px;">Scanning Package Archives...</div>
                <div style="font-size: 12px; opacity: 0.8; margin-top: 4px;">Decompressing files & reading native C++ (.so) symbols...</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            # Step 2: Update Scanner Animation
            scanner_placeholder.markdown("""
            <div class="scanner-box">
                <div class="pulse-ring"></div>
                <div style="font-weight: bold; font-size: 15px;">Computing Deep Diffs...</div>
                <div style="font-size: 12px; opacity: 0.8; margin-top: 4px;">Categorizing feature flags, endpoints, and background services...</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            added_sdks = list(new_data["detected_sdks"] - old_data["detected_sdks"])
            ...
            === 4. NEW BACKGROUND SERVICES & RECEIVERS ({len(added_services)}) ===
            {added_services[:20]}
            ...
            === 6. NEW THIRD-PARTY SDKs ({len(added_sdks)}) ===
            {added_sdks}
            
            === 7. SECURITY & PERMISSIONS ({len(added_permissions)}) ===
            {added_permissions[:20]}
            
            === 8. NATIVE C++ BINARY TOKENS (.so) ({len(added_native)}) ===
            {added_native[:35]}
            
            === 9. GAME CONFIGS & DATA TABLES ({len(added_configs)}) ===
            {added_configs[:35]}
            
            === 10. FILE PATH DIFFS ===
            ADDED ({len(added_files)}): {added_files[:20]}
            REMOVED ({len(removed_files)}): {removed_files[:20]}
            """
            ...
            <div class="scanner-box">
                ...
                <div style="font-size: 12px; opacity: 0.8; margin-top: 4px;">Synthesizing findings into Material Design 3 dashboard...</div>
            </div>
            """, unsafe_allow_html=True)
            ...
            Examine the provided package diffs and format your response into a clean, modern Material Design 3 (MD3) report dashboard.
            ...
            1. **Top Metric Chips Row**: Highlighting Size Change, Impact Rating (e.g. 8/10), New Flags Count, and New Endpoints Count.
            2. **Target Keyword Findings**: If custom keywords were found, detail them prominently at the top.
            3. **Executive Teardown Verdict**: Comprehensive summary explaining unreleased features, background services, and SDK changes.
            ...
            5. **Security, Endpoints & Infrastructure Audit**: Highlight new server endpoints (`https://`), deep links, and permissions.
            ...
            try:
                ...
                # Store in session state and switch to Fullscreen Report view
